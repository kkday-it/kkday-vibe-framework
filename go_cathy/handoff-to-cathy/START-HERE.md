...
|---|---|---|
| 1 | **提供 Supabase 存取**（連線字串 + Dashboard 登入 + access token）——只給你自己的 Claude 用 | **照 `G0-EXPORT-GUIDE.md` 手動 Dashboard 路徑**帶你匯出 9 樣（SQL 貼 SQL Editor 跑、Edge Fn/Auth/Storage 畫面複製、存檔、驗證） |
...
> 「照 `G0-EXPORT-GUIDE.md` 的**手動 Dashboard 路徑**一步步帶我匯出：SQL 我貼到 Supabase 的 **SQL Editor** 跑、Edge Function / Auth / Storage 你教我在畫面上點。**我的專案 ref、access token、DB 連線字串只給你用、不外傳。**」

你只要：
- 手上備好 **專案 ref + access token + DB 連線字串 + Dashboard 登入**（指南開頭有列在哪拿）——🔒 **這些只給你自己的 Claude 蒐集用，不用傳給 Lance 或任何人**。
- 照 AI 指示，把它給的 SQL 貼到 Supabase **SQL Editor** 跑、下載結果；要在畫面點/截圖的（Edge Function、Auth、Storage）照做。
- 匯出完，把 **結果檔**（`supabase-export/` 那包技術定義）交給 RD——裡面不含你的登入/token。
- ⚙️ **不用你跑「自動盤點 skill」**：那要能跑程式的 AI（RD 端的 Claude Code / coding agent），聊天版 Claude Desktop 跑不了。**你走手動 Dashboard 路徑就好**；skill 是 RD 有 coding agent 時的加速選項。
