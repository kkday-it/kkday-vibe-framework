...
## 留痕紀律（每個動作都要能事後 retro）

**這個交接的每一步——不管 AI 還是人做——都要留下可事後回顧的紀錄。** 目的：G0 / 重構 / UAT 任一步出問題時，能重建「當時做了什麼、哪裡失敗」，而不是靠記憶。

- **跑指令一律留 console log**：`<cmd> 2>&1 | tee <階段>.log`（很多工具的階段訊息走 stderr，不 tee 就不留檔）。
- **保留原始輸出，不要只留結論**：G0 留 `raw/**`、`manifest.json`、`_status.tsv` 與各 `.err`；② 重構留「改了哪些檔 / 跑了哪些驗證 / 結果」；⑤ UAT 留每項 pass/fail 與截圖。
- **失敗也要留**：查詢/指令失敗的 `.err` 或錯誤原文原封保留——**空結果不等於「沒有」**，`.err` 才分得出「查失敗 vs 真的沒有」。
- **每階段收尾記一筆**：日期、做了什麼、遇到什麼、下一步交給誰——供 retro 與交接。
