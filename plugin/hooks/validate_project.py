#!/usr/bin/env python3
"""守門檢查(guard.yml 與 vibe-governance plugin Stop hook 共用):
PROJECT.yaml schema、workflows manifest、禁用套件。
gitleaks 由 workflow 另跑。exit 0=過,1=紅燈。

--project-root:打包進 plugin 後 __file__ 位於 plugin cache,不能再用
腳本位置推導專案根;hook 情境一律顯式傳入(spec 2026-09-14 H2)。"""
import argparse
import re
import sys
...
def tiered(risk_tier: str, msg: str):
    """cloud-ready 檔案檢查依 tier 分級:green=warning、yellow+=error(spec 2026-09-14 H5)。"""
    if risk_tier in ("yellow", "red"):
        errors.append(msg)
    else:
        warnings.append(msg + "(green 級為提醒;要共用/排程/上雲時必須補)")
...
def check_cloud_ready_spec(data: dict):
    tier = str(data.get("risk_tier", "green"))
    # 1. 必備檔案缺失(green=warning、yellow+=error)
    if not (ROOT / "Dockerfile").exists():
        tiered(tier, "Cloud-Ready: 根目錄缺少 Dockerfile")
    if not (ROOT / ".dockerignore").exists():
        tiered(tier, "Cloud-Ready: 根目錄缺少 .dockerignore")
    if not (ROOT / "compose.yml").exists() and not (ROOT / "docker-compose.yml").exists():
        tiered(tier, "Cloud-Ready: 根目錄缺少 compose.yml(第一關驗收要求,spec §1.8/§4.10)")

    # repo 名全小寫 kebab-case（spec §4.10）:registry(ECR 等)拒收大寫,第一次 push 就會被擋
    if not re.fullmatch(r"[a-z0-9-]+", ROOT.name):
        tiered(tier, f"Cloud-Ready: repo 目錄名 '{ROOT.name}' 不是全小寫 kebab-case（僅准 a-z、0-9、連字號；registry 拒收大寫，spec §4.10）")
...
def main() -> int:
    ...
    ap.add_argument("--project-root", help="專案根目錄;省略時依腳本位置推導(repo 內執行用)。plugin/hook 情境必填。")
    args = ap.parse_args()
